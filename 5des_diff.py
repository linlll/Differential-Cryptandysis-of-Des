...
class DifferentialCryptandysis:
  ...
  def guessP(self, Sxor): # 调试程序，用于找出合适的明文差分
    for i in range(8):
      R0 = '0' * i + '6' + (7 - i) * '0' # 2、4、6
      R0 = hex2bin(R0, 32)

      sx = Sxor[i][bin2int(self.des.E(R0)[i*6:(i+1)*6])]
      m = int2hex(np.argmax([len(s) for s in sx]), 1)
      R0_OUT = hex2bin('0' * i + m + (7 - i) * '0', 32)
      R0_OUT = [R0_OUT[p-1] for p in P_Table]

      L0 = R0_OUT
      R3 = R0_OUT

      print('L0: {}, R0: {}, R3: {}'.format(bin2hex(L0, 8), bin2hex(R0, 8), bin2hex(R3, 8)))
  ...
  def analyze_single(self, dP, pos, kd_i):
    ...
    LL5, RR5 = copy.deepcopy(TT[0:32]), copy.deepcopy(TT[32:64])
...
if __name__ == '__main__':
  ...
  print('\n==> Built a Des!')
  dc = DifferentialCryptandysis(des, 2**6, 10)
  start = time.time()
  key = dc.analyze()
  end = time.time()
