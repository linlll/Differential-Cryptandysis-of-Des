...
class DifferentialCryptandysis:
  def __init__(self, des: Des, M, testnum=5) -> None:
    ...
    self.dP = ['0401000000600000', '2000008000006000']
    self.dR2 = ['00600000', '00006000']
    self.prob_key = [[] for _ in range(8)]
    self.key_dict = [defaultdict(int), defaultdict(int)]
    self.key_pos = [[0,2,5,6,7], [1,2,3,4,7]]
  ...
  def analyze_single(self, dP, pos, kd_i):
    ...
    dR6 = xor(R6, RR6)
    OUT = xor(dR6, hex2bin(self.dR2[kd_i], 32))
...
if __name__ == '__main__':
  ...
  print('\n==> Built a Des!')
  dc = DifferentialCryptandysis(des, 2**9, 10)
  start = time.time()
  key = dc.analyze()
  end = time.time()
